"""
51% hashrate pool with multiple nodes - attack-multi51.py
Copyright (C) 2017 Yeon-Soo Kim <alkorang@outlook.com>
(Derived from btcsim/attack-51.py)
...
# setup bad miners
attacker_strength = 0.51 / numbads
for i in range(numbads):
    hashrates[i] = 0.0
bad_hashrate = hashrates.sum() * (attacker_strength/(1.0 - attacker_strength * numbads))
for i in range(numbads):
    hashrates[i] = bad_hashrate

hashrates = hashrates/hashrates.sum()
miners = []
for i in range(numbads):
    badminer = BadMiner(i, hashrates[i] * 1.0/600.0, 200*1024, seed_block, event_q, t)
    miners.append(badminer)
for i in range(numbads, numminers):
     miner = Miner(i, hashrates[i] * 1.0/600.0, 200*1024, seed_block, event_q, t)
     miners.append(miner)
